...
void destroy_remaining_heap_allocations(Matrix_t **mats, unsigned int num_mats);
	...
	/*
	* PURPOSE: accepts inputs from the user to be used in creating matrices, then frees everything from memory
	* INPUT: user arguments
	* RETURN: if everything runs successfully, returns a 0 to indicate the end of the program was reached. If an error occurs a -1 is returned
	*/
int main (int argc, char **argv) {
	...
	if (add_matrix_to_array(mats,temp, 10) < 0) { //TODO ERROR CHECK NEEDED
		printf("There's no matrix here part 2: Return of the lack of matrix\n");
		return -1;
	}
	...
	if (!write_matrix("temp_mat", mats[mat_idx])) { // TODO ERROR CHECK
		printf("Couldn't write this matrix\n");
		return -1;
	}
	...
	//TODO FUNCTION COMMENT
	/*
	* PURPOSE: parses through commands and then executes them (performs operations on the matrices)
	* INPUTS: command pointer, array of matrices, amount of matrices in the array
	* RETURN: Void function, so doesn't return anything, may return prematurely if the matrix does not exist or cannot be created
	*/
void run_commands (Commands_t* cmd, Matrix_t** mats, unsigned int num_mats) {
	...
	if (strncmp(cmd->cmds[0],"display",strlen("display") + 1) == 0
		...
		if (mat1_idx >= 0 ) {
				...
				if (!duplicate_matrix (mats[mat1_idx], dup_mat)) { //TODO ERROR CHECK NEEDED
					printf("The matrix was not duplicated\n");
					return;
				}
				if (add_matrix_to_array(mats,dup_mat,num_mats) < 0) {//TODO ERROR CHECK NEEDED
					printf("Matrix was not added to array");
					return;
				}
		...
		if (mat1_idx >= 0 ) {
			if (!bitwise_shift_matrix(mats[mat1_idx],cmd->cmds[2][0], shift_value)) {//TODO ERROR CHECK NEEDED
				printf("Something went wrong; no bits were shifted\n")
				return;
		}
		...
		if (add_matrix_to_array(mats,new_matrix, num_mats) < 0) { //TODO ERROR CHECK NEEDED
			printf("Could not add matrix to the array\n");
			return;
		}
		...
		if (add_matrix_to_array(mats,new_mat,num_mats) < 0) { // TODO ERROR CHECK NEEDED
			printf("Could not add matrix tot he array");
			return;
		}
		...
		if (!random_matrix(mats[mat1_idx],start_range, end_range)) {//TODO ERROR CHECK NEEDED
			printf("Random matrix was not created\n");
			return;
		}
	...
	//TODO FUNCTION COMMENT
	/*
	PURPOSE: Given a name, this function searches through the matrix array for a matrix that matches the name
	INPUTS: matrix array pointer, amount of matrices, matrix to find
	RETURN: If the search is successfull, returns the index of the matrix that was searched for, otherwise returns -1 to indicate an error
	*/
unsigned int find_matrix_given_name (Matrix_t** mats, unsigned int num_mats, const char* target) {
	//TODO ERROR CHECK INCOMING PARAMETERS
	if (!mats || num_mats < 1 || !target) {
		printf("Recheck your inputs\n");
		return -1;
	}
	...
	//TODO FUNCTION COMMENT
	/*
	* PURPOSE: frees all memory associated with the matrix array
	* INPUTS: pointer to matrices, amount of matrices
	* RETURN: Void function, so no actual return.
	*/
void destroy_remaining_heap_allocations(Matrix_t **mats, unsigned int num_mats) {
